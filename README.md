> **第三轮修订状态：原12-run自然流程方案已暂停。**现已改成暴露受控的银行入口实验；尚未调用新批次。历史暴露为0/44；本实验不能用于估计自然冲突发生/暴露概率。实现说明见MODEL_DECISIONS.md末节，输入与预算见results/framing_v3_controlled_preflight。
